geometries[GUI_GEOMETRY_BLOB] = new (function()
{ 
    var normals = [];
...
Model.definitions[Model.GUI_DEFINITION] = function()
{	

    this.geometries = [GUI_GEOMETRY_BLOB];

	this.normals = geometries[GUI_GEOMETRY_BLOB].normals;                                                  
                                                  
	this.geometry = geometries[GUI_GEOMETRY_BLOB].vertices;
    
	this.texcoords = geometries[GUI_GEOMETRY_BLOB].texcoords;
    
	this.indexes = geometries[GUI_GEOMETRY_BLOB].indexes;		
    
	this.MIDDLE = 0;
		
	this.accents = [];
	
    this.pointer_accent = this.accents[0] = {filter:[0, 1, 0],  indexes: [0, 1, 2, 3]};
...
//                                                                         37, 43, 43, 37, 42, 39, 38, 44, 44, 38, 43, 40, 39, 45, 45, 39, 44, 41, 36, 23, 23, 36, 5]}]
	...
	this.accents = [{filter:[0, 1, 0], base:[], indexes: [0, 1, 2, 2, 1, 3]}]

/*    this.mapping_length = 1;   
    this.mapping = [ { schematic: 2, orbit: 6, stroke: 0 } ]; */           

    this.mapping_length = 0;   
    this.mapping = [ ];
...
//        [ Model.LOWER_LEG_DEFINITION, [20, 50, 20], [[14, -25, 0]]]
    ...
    this.patterns = [Model.GUI_DEFINITION];
};
